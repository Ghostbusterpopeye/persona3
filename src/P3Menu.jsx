...
const ITEMS = [
  { id: "about",   label: "ABOUT ME",    page: "about",   fontSize: 130, offsetX: 0,  offsetY: 0  },
  { id: "resume",  label: "RESUME",      page: "resume",  fontSize: 108, offsetX: 38, offsetY: -8 },
  { id: "github",  label: "GITHUB LINK", page: null,      fontSize: 88,  offsetX: 14, offsetY: -6,
    externalUrl: "https://github.com/Ghostbusterpopeye" },
];
...
export default function P3Menu({ onNavigate }) {
  ...
  // Satu fungsi terpusat untuk handle semua item
  const handleSelect = (item) => {
    if (item.externalUrl) {
      // Buka tab baru, TIDAK navigate internal
  ...
  return (
    <>
      ...
      <div className="p3-root">
        <video className="p3-video" src="/Mainn.mp4" autoPlay loop muted playsInline />
          ...
          {ITEMS.map((item, i) => {
            ...
            return (
              ...
              >
                ...
                />
                <span className="p3-label" style={{ fontSize: item.fontSize, opacity }}>
                  {item.label}
                </span>
                {item.externalUrl && (
                  <span className="p3-external-badge">↗</span>
                )}
              </a>
            );
          })}
